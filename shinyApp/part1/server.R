# The code creats tables in the global scope  showing results of three binary 
...
# ATTACH DATAFRAME TO MAKE OBJECTS WITHIN IT ACCESSIBLE IN R
##########################################################################
...
t.ct <- ctree(as.factor(survived) ~ pclass + sex + age + sibsp, 
             data=mydata.nomiss, controls = ctree_control(mincriterion = 0.95))
