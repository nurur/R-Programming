#The code contains Part A and B of Homework Assignment 2
#
#Part A
#Load required libraries
#Score 7 differnt airlines using passenger's sentiment
#Create two tables: twitterData.df, acsiData.df
...
#Use 'mydata' database   
#Write tables, twitterData and acsiData inside the database
...
workDir     = '/Users/nurur/ugw/dnsc6205_computation/'
# Locate the lexicon files within the directory 
posTextFile = paste(workDir, 'positive-words.txt', sep='') 
negTextFile = paste(workDir, 'negative-words.txt', sep='') 
...
#Connect to the MySQL Server
#Use 'mydata' database 
#Write two tables: twitterData and acsiData inside the database

library(RMySQL)
con <- dbConnect('MySQL', 'mydata', user="root", password="mysql", host="localhost")
#dbListTables(con)
#dbListFields(con, 'myTable')
#dbReadTable(con,  'myTable')

dbWriteTable(con, "twitterData", twitter.df)
dbWriteTable(con, "acsiData", acsi.df)
dbDisconnect(con)
